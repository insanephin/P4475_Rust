...
use p4475_core::ports::{DEFAULT_CONFIGURED_PORT, PortTopology};
use p4475_server::{
...
#[command(
    name = "p4475",
    version,
    about = "Cross-platform KartRider P4475 server and connector"
)]
struct Cli {
...
enum Command {
    /// Run the P4475 server and its optional XUN sidecar endpoint.
    Server(ServerArgs),
...
struct ConnectArgs {
    ...
    /// Request the P4475 observer-master login role (pmap 718).
    #[arg(long, conflicts_with = "anonymous_league")]
    observer: bool,

    /// Request the recovered P4475 anonymous-league profile (pmap 1798).
...
fn init_tracing(verbose: bool) -> Result<LoggingRuntime> {
    ...
    let packet_file = tracing_subscriber::fmt::layer()
        ...
        .with_filter(EnvFilter::new("info,p4475_packet=debug"));
...
fn create_log_file() -> Result<PathBuf> {
    let directory = match std::env::var_os("P4475_LOG_DIR") {
...
fn create_log_file_in(directory: &Path, timestamp: u128, process: u32) -> Result<PathBuf> {
    ...
    for sequence in 0_u8..=99 {
        ...
        let path = directory.join(format!("p4475-{timestamp}-{process}{suffix}.log"));
    ...
    bail!(
        "could not reserve a unique P4475 log file in {}",
        directory.display()
    )
}

fn default_log_directory() -> Result<PathBuf> {
    let executable = std::env::current_exe().context("failed to locate the p4475 executable")?;
    let parent = executable
        .parent()
        .ok_or_else(|| anyhow::anyhow!("p4475 executable has no parent directory"))?;
    Ok(parent.join("logs"))
}

async fn run_server(args: ServerArgs) -> Result<()> {
    let ports = PortTopology::new(args.configured_port)
        .context("configured port cannot provide all P4475 service offsets")?;
    ...
    let server = BoundServer::bind(config)
        .await
        .context("failed to bind the P4475 transport set")?
        .start()
        .context("failed to start the P4475 supervisor")?;
    ...
    tracing::info!(
        ...
        "P4475 foundation server is running"
    );
...
async fn wait_for_server_exit(server: &ServerHandle) -> Result<()> {
    tokio::select! {
        result = server.wait() => result.context("P4475 server runtime stopped"),
    ...
    match server.shutdown().await {
        ...
        Err(
            ...
            tokio::select! {
                result = server.wait() => {
                    result.context(
                        "P4475 server runtime stopped while awaiting force-shutdown confirmation",
                    )
                }
                signal = shutdown_signal() => {
...
mod tests {
    ...
    fn no_arguments_select_gui_and_any_argument_selects_cli() {
        assert!(super::should_start_gui([OsString::from("p4475")]));
    ...
    fn server_uses_legacy_profile_directory_and_no_catalog_by_default() {
        let cli = Cli::try_parse_from(["p4475", "server"]).unwrap();
        ...
        assert_eq!(
            ...
            p4475_server::DEFAULT_MAX_LOGIN_SESSIONS
        );
        assert!(args.trust_client_item_rank);
        assert!(!args.allow_remote_profile_creation);

        let cli =
            Cli::try_parse_from(["p4475", "server", "--trust-client-item-rank", "false"]).unwrap();
        let Some(Command::Server(args)) = cli.command else {
            panic!("server subcommand should parse as Command::Server");
        };
        assert!(!args.trust_client_item_rank);
    ...
    fn sikarugir_runner_and_custom_executable_parse_from_cli() {
        ...
        assert!(matches!(
            super::build_runner(&args).unwrap(),
            p4475_connector::Runner::Sikarugir { app }
    ...
    fn file_log_reservation_creates_distinct_appendable_files() {
        ...
        assert_eq!(first.file_name().unwrap(), "p4475-123-456.log");
        assert_eq!(second.file_name().unwrap(), "p4475-123-456-1.log");
