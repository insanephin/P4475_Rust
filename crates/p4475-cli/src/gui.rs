...
use p4475_core::{
    floater_physics::{ALL_FLOATER_CODES, p4475_floater_spec},
...
const WINDOW_TITLE: &str = "KartRider P4475";
const BUILD_VERSION: &str = env!("CARGO_PKG_VERSION");
const GUI_CLOSE_GRACE_PERIOD: Duration = Duration::from_secs(5);
const GUI_SETTINGS_KEY: &str = "p4475-gui-settings-v2";
...
pub(crate) fn run(logging: &LoggingRuntime) -> Result<()> {
    ...
    eframe::run_native(
        ...
        Box::new(move |creation_context| {
            ...
            Ok(Box::new(P4475GuiApp::new_with_logging(
                log_path,
                logging_control,
...
impl Default for GuiInputs {
    fn default() -> Self {
        Self {
            ...
            crossover_bottle: "KartRider-P4475".to_owned(),
...
impl GuiInputs {
    fn connector_plan(&self, language: GuiLanguage) -> Result<ConnectorPlan> {
        ...
        let launcher_profile_role = match (self.observer_mode, self.anonymous_league_mode) {
            (false, false) => p4475_connector::LauncherProfileRole::Regular,
            (true, false) => p4475_connector::LauncherProfileRole::ObserverMaster,
            (false, true) => p4475_connector::LauncherProfileRole::AnonymousLeague,
            (true, true) => {
                return Err(anyhow!(tr!(
                    language,
        ...
        ConnectorPlan::new(ConnectorRequest {
            ...
            ports,
            runner,
            probe_timeout: p4475_connector::DEFAULT_PROBE_TIMEOUT,
...
impl GuiPersistedSettings {
    ...
    fn from_app(app: &P4475GuiApp) -> Self {
        Self {
...
impl Default for ServerInputs {
    fn default() -> Self {
        Self {
            ...
            max_login_sessions: p4475_server::DEFAULT_MAX_LOGIN_SESSIONS.to_string(),
...
impl ServerInputs {
    ...
    fn server_config(&self, language: GuiLanguage) -> Result<ServerConfig> {
        ...
        let ports = PortTopology::new(configured_port).with_context(|| {
            tr!(
                language,
                "기준 포트에서 P4475 서비스 포트를 모두 만들 수 없습니다",
                "The P4475 service ports cannot be derived from the base port",
                "无法从基准端口生成全部 P4475 服务端口"
            )
        })?;
        ...
        let speed_basic_ai_parameters =
            parse_basic_ai_parameters(&self.speed_basic_ai_parameters, language).with_context(
                || {
                    tr!(
                        language,
                        "스피드전 AI 파라미터가 P4475 패킷의 허용 범위를 벗어났습니다",
                        "A speed-mode AI parameter is outside the P4475 packet limits",
                        "竞速模式 AI 参数超出 P4475 数据包允许范围"
                    )
                },
            )?;
        let item_basic_ai_parameters =
            parse_basic_ai_parameters(&self.item_basic_ai_parameters, language).with_context(
                || {
                    tr!(
                        language,
                        "아이템전 AI 파라미터가 P4475 패킷의 허용 범위를 벗어났습니다",
                        "An item-mode AI parameter is outside the P4475 packet limits",
                        "道具模式 AI 参数超出 P4475 数据包允许范围"
                    )
                },
            )?;
...
fn floater_code_label(language: GuiLanguage, code: i16) -> String {
    ...
    // Exact group/id names from the stock Korean P4475
...
fn localized_track_candidate_reason(reason: &str, language: GuiLanguage) -> String {
    match reason {
        "already installed in the selected P4475 DataRaw tree" => tr!(
            language,
            "선택한 P4475 DataRaw에 이미 설치됨",
            "Already installed in the selected P4475 DataRaw tree",
            "已安装到所选 P4475 DataRaw"
        )
        .to_owned(),
...
fn localized_asset_candidate_reason(reason: &str, language: GuiLanguage) -> String {
    match reason {
        "requires item-result rules that do not exist in P4475" => tr!(
            language,
            "P4475에 없는 신규 아이템 결과 규칙이 필요함",
            "Requires new item-result rules unavailable in P4475",
            "需要 P4475 中不存在的新道具结果规则"
        )
        .to_owned(),
...
impl P4475GuiApp {
                    ...
                    match result {
                        Ok(candidates) => {
                            ...
                            self.asset_import_status = tr_format!(
                                self.language,
                                "감사 완료: 가져오기 가능 {}개, P4475 미지원 아이템 규칙으로 제외 {}개.",
                                "Audit complete: {} importable; {} excluded for unsupported P4475 item rules.",
                                "审计完成：可导入 {} 个；因 P4475 不支持的道具规则而排除 {} 个。",
                                eligible,
                                blocked
                            );
    ...
    fn start_connector(&mut self, context: &egui::Context) {
        ...
        if let Err(error) = thread::Builder::new()
            .name("p4475-connector-worker".to_owned())
            .spawn(move || {
                let outcome =
    ...
    fn start_xun_attach(&mut self, context: &egui::Context) {
        ...
        if let Err(error) = thread::Builder::new()
            .name("p4475-xun-attach-worker".to_owned())
            .spawn(move || {
    ...
    fn start_track_candidate_scan(&mut self, context: &egui::Context) {
            ...
            || game_root.as_os_str().is_empty()
        {
            tr!(
                self.language,
                "소스 Data 폴더와 P4475 게임 폴더를 모두 지정하세요.",
                "Specify both the source Data directory and the P4475 game directory.",
                "请同时指定源 Data 目录和 P4475 游戏目录。"
            )
        ...
        let cache = game_root.join(".p4475-track-import").join("cache");
        ...
        if let Err(error) = thread::Builder::new()
            .name("p4475-track-scan-worker".to_owned())
            .spawn(move || {
    ...
    fn start_track_import(&mut self, context: &egui::Context) {
        ...
        let game_root = PathBuf::from(self.connector_inputs.game_directory.trim());
        let workspace = game_root.join(".p4475-track-import");
        ...
        if let Err(error) = thread::Builder::new()
            .name("p4475-track-import-worker".to_owned())
            .spawn(move || {
                let mut last_progress = None;
                let mut on_progress = |progress| {
                    send_track_progress_throttled(&notifier, &mut last_progress, progress);
    ...
    fn start_asset_candidate_scan(&mut self, context: &egui::Context) {
        ...
        let target_data_raw = game_root.join("DataRaw");
        let cache = game_root.join(".p4475-asset-import").join("cache");
        ...
        if let Err(error) = thread::Builder::new()
            .name("p4475-asset-scan-worker".to_owned())
            .spawn(move || {
                let data_raw = target_data_raw
                    .is_dir()
                    .then_some(target_data_raw.as_path());
    ...
    fn start_asset_import(&mut self, context: &egui::Context) {
        ...
        let game_root = PathBuf::from(self.connector_inputs.game_directory.trim());
        let workspace = game_root.join(".p4475-asset-import");
        ...
        if let Err(error) = thread::Builder::new()
            .name("p4475-asset-import-worker".to_owned())
            .spawn(move || {
                let mut last_progress = None;
                let mut on_progress = |progress| {
                    send_asset_progress_throttled(&notifier, &mut last_progress, progress);
...
impl P4475GuiApp {
    fn start_server(&mut self, context: &egui::Context) {
        ...
        match thread::Builder::new()
            .name("p4475-server-worker".to_owned())
            .spawn(move || {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        egui::ComboBox::from_id_salt(("grant-floater-slot", slot))
                            ...
                            .show_ui(ui, |ui| {
                                ...
                                if p4475_floater_spec(none_candidate).is_some() {
                                    ui.selectable_value(
                                        &mut selected,
                                        0,
                                ...
                                for &code in ALL_FLOATER_CODES {
                                    ...
                                    candidate[slot] = code;
                                    if p4475_floater_spec(candidate).is_some() {
                                        ui.selectable_value(
                                            &mut selected,
                                            code,
    ...
    fn item_probability_editor(&mut self, ui: &mut egui::Ui) {
        ...
        ui.collapsing(
            ...
            ui.weak(tr!(
                language,
                "아이템 플로터 확률은 한국 P4475 enchant.xml의 코드별 원본 값을 사용합니다.",
                "Item Floater chances use the per-code stock Korean P4475 enchant.xml values.",
                "道具强化属性概率使用韩服 P4475 enchant.xml 中各代码的原始数值。"
            ));
            let pinned =
                self.server_inputs.item_probability_source == GuiItemProbabilitySource::Edited;
    ...
    fn server_input_panel(&mut self, ui: &mut egui::Ui) {
        ...
        ui.weak(tr!(
            language,
            "주소에는 IP 리터럴만 사용할 수 있습니다. P4475 패킷은 광고 주소를 IPv4 4바이트로 기록하므로 도메인을 직접 넣을 수 없습니다.",
            "Addresses must be IP literals. P4475 stores the advertised address as four IPv4 bytes, so a domain name cannot be entered directly.",
            "地址必须是 IP 字面值。P4475 数据包以 4 字节 IPv4 保存公布地址，因此不能直接输入域名。"
        ));
        ui.weak(tr!(
            language,
    ...
    fn server_tab(&mut self, ui: &mut egui::Ui) {
        ...
        ui.label(tr!(
            language,
            "P4475 서버를 설정하고 클라이언트가 접속하는 동안 실행합니다.",
            "Configure the P4475 server and keep it running while clients connect.",
            "配置 P4475 服务器，并在客户端连接期间保持运行。"
        ));
        ui.add_space(10.0);
    ...
    fn basic_ai_parameters_editor(&mut self, ui: &mut egui::Ui) {
        ...
        ui.collapsing(
            ...
            |ui| {
                ui.weak(tr!(
                    language,
                    "스피드전과 아이템전에 각각 적용됩니다. 앞의 네 값은 실제 주행에 쓰이고, 마지막 두 값은 P4475 호환용 예약 필드입니다.",
                    "Speed and item races use separate vectors. P4475 consumes the first four values while the final two are compatibility fields.",
                    "竞速赛和道具赛分别使用独立参数。P4475 使用前四项，最后两项为兼容保留字段。"
                ));
                ui.add_space(4.0);
                let labels = [
    ...
    fn track_import_tab(&mut self, ui: &mut egui::Ui) {
        ...
        ui.label(tr!(
            language,
            "외부 클라이언트의 RHO/RHO5를 직접 읽어 트랙을 고르고, 트랙 파일·AI 경로·테마·썸네일·재질·환경음·BGM 의존성을 검증한 뒤 현재 P4475 DataRaw에 추가합니다.",
            "Reads an external client's RHO/RHO5 files, lets you select tracks, audits track files, AI paths, themes, thumbnails, materials, environment sounds, and BGM, then adds the verified closure to this P4475 DataRaw tree.",
            "直接读取外部客户端的 RHO/RHO5，选择赛道，审计赛道文件、AI 路径、主题、缩略图、材质、环境音和 BGM，然后将验证后的完整依赖添加到当前 P4475 DataRaw。"
        ));
        ui.add_space(8.0);
        egui::Grid::new("track-import-inputs")
            ...
            .show(ui, |ui| {
                ...
                ui.label(tr!(language, "대상 P4475", "Target P4475", "目标 P4475"));
    ...
    fn asset_import_tab(&mut self, ui: &mut egui::Ui) {
        ...
        ui.label(tr!(
            language,
            "정적 분석과 과거 실행 시험을 통과한 후보만 표시합니다. XUN 엔진 카트는 목록에서 제외하며, P4475에 없는 새 아이템 결과 규칙을 요구하는 카트는 비활성화합니다.",
            "Only candidates that passed the static audit and prior runtime trials are shown. XUN karts are omitted, and karts requiring item-result rules absent from P4475 are disabled.",
            "仅显示已通过静态审计和既往运行测试的候选项。XUN 车辆不会列出；需要 P4475 中不存在的新道具结果规则的车辆将被禁用。"
        ));
        ui.add_space(8.0);
    ...
    fn xun_attach_file_panel(&mut self, ui: &mut egui::Ui) {
        ...
        egui::Grid::new("xun-attach-files")
            ...
            .show(ui, |ui| {
                ...
                .on_hover_text(tr!(
                    language,
                    "기본값은 접속기와 같은 폴더의 p4475-xun-attach.exe입니다.",
                    "Defaults to p4475-xun-attach.exe beside the connector.",
                    "默认使用连接器同目录下的 p4475-xun-attach.exe。"
                ));
                if ui
                ...
                .on_hover_text(tr!(
                    language,
                    "기본값은 접속기와 같은 폴더의 p4475-xun.dll입니다.",
                    "Defaults to p4475-xun.dll beside the connector.",
                    "默认使用连接器同目录下的 p4475-xun.dll。"
                ));
                if ui
                    .button(tr!(language, "찾아보기", "Browse", "浏览"))
                    .clicked()
    ...
    fn connector_tab(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            .on_hover_text(tr!(
                language,
                "끄면 XUN 기능은 유지하지만 p4475-xun-sidecar.log 파일을 기록하지 않습니다.",
                "When disabled, XUN features remain active but p4475-xun-sidecar.log is not written.",
                "关闭后仍启用 XUN 功能，但不会写入 p4475-xun-sidecar.log。"
            ));
        });
        ui.add_space(10.0);
...
impl eframe::App for P4475GuiApp {
...
fn run_server_worker(
    ...
    tracing::info!(
        ...
        "GUI requested P4475 server startup"
    );
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
    ...
    runtime.block_on(async move {
        let server = BoundServer::bind(config)
            ...
            .with_context(|| {
                tr!(
                    language,
                    "P4475 네트워크 포트를 열지 못했습니다",
                    "Failed to open the P4475 network ports",
                    "无法打开 P4475 网络端口"
                )
            })?
            .start()
            .with_context(|| {
                tr!(
                    language,
                    "P4475 서버 감독 작업을 시작하지 못했습니다",
                    "Failed to start the P4475 server supervisor",
                    "无法启动 P4475 服务器监督任务"
                )
            })?;
...
async fn run_server_control_loop(
    server: &p4475_server::ServerHandle,
    ...
    loop {
        tokio::select! {
            result = server.wait() => return result.with_context(|| tr!(language, "P4475 서버 런타임이 종료되었습니다", "The P4475 server runtime exited", "P4475 服务器运行时已退出")),
...
const XUN_ATTACH_HELPER_NAME: &str = "p4475-xun-attach.exe";
const XUN_ATTACH_DLL_NAME: &str = "p4475-xun.dll";
...
fn write_xun_attach_configuration(files: &XunAttachFiles, logging: bool) -> Result<()> {
    ...
    let path = directory.join("p4475-xun.ini");
    let contents = format!(
...
fn run_xun_attach_elevated(
    ...
    const SCRIPT: &str = concat!(
        ...
        "$quotedDll = '\"' + $env:P4475_XUN_ATTACH_DLL + '\"'; ",
        "$arguments = if ([string]::IsNullOrWhiteSpace($env:P4475_XUN_ATTACH_PID)) ",
        "{ $quotedDll } else { $env:P4475_XUN_ATTACH_PID + ' ' + $quotedDll }; ",
        "$process = Start-Process -FilePath $env:P4475_XUN_ATTACH_EXE ",
        "-ArgumentList $arguments -WorkingDirectory $env:P4475_XUN_ATTACH_DIR ",
        "-Verb RunAs -WindowStyle Hidden -Wait -PassThru; ",
        "[Console]::Out.WriteLine('P4475_XUN_ATTACH_EXIT=' + $process.ExitCode); ",
    ...
    let output = Command::new(&powershell)
        ...
        .env("P4475_XUN_ATTACH_EXE", &files.helper)
        .env("P4475_XUN_ATTACH_DLL", &files.dll)
        .env("P4475_XUN_ATTACH_DIR", working_directory)
        .env(
            "P4475_XUN_ATTACH_PID",
...
mod tests {
    ...
    use p4475_connector::{ConnectorCancellation, ConnectorStage, RunnerBackend};
    use p4475_server::{
    ...
    use super::{
        ...
        P4475GuiApp, ServerInputs, TrackImportInputs, XUN_ATTACH_DLL_NAME, XUN_ATTACH_HELPER_NAME,
    ...
    use p4475_assets::{AssetCategory, TrackSourceRegion};
    ...
    fn fixture_inputs() -> GuiInputs {
        GuiInputs {
            ...
            data_pack_off: true,
            xun_attach_helper: "/tools/p4475-xun-attach.exe".to_owned(),
            xun_attach_dll: "/tools/p4475-xun.dll".to_owned(),
            ...
            wine_prefix: "/bottles/p4475".to_owned(),
            crossover_binary: "/opt/cxoffice/bin/wine".to_owned(),
            crossover_bottle: "P4475".to_owned(),
    ...
    fn xun_attach_files_default_to_the_connector_directory() {
        ...
        let application = directory.path().join("p4475.exe");
    ...
    fn xun_attach_configuration_controls_only_logging_and_keeps_hooks_enabled() {
        ...
        let contents = fs::read_to_string(directory.path().join("p4475-xun.ini")).unwrap();
    ...
    fn inputs_build_the_same_non_mutating_connector_plan_as_cli() {
        ...
        assert_eq!(
            ...
            p4475_connector::LauncherProfileRole::ObserverMaster
        );
        ...
        assert!(plan.prepared_paths().iter().any(|path| {
            path.ends_with(Path::new("Data").join(p4475_connector::SPECIAL_TRACK_OVERLAY_FILE))
        }));
        ...
        assert_eq!(
            ...
            [("WINEPREFIX".into(), "/bottles/p4475".into())]
        );
    }
    ...
    fn account_role_checkboxes_map_to_explicit_profiles() {
        ...
        assert_eq!(
            inputs
                ...
                .installation_options
                .launcher_profile_role,
            p4475_connector::LauncherProfileRole::Regular
        );
        ...
        assert_eq!(
            inputs
                ...
                .installation_options
                .launcher_profile_role,
            p4475_connector::LauncherProfileRole::AnonymousLeague
        );
    ...
    fn gui_persists_server_and_connector_inputs_between_runs() {
        let mut app = P4475GuiApp::new(PathBuf::new(), None);
        ...
        app.server_inputs = ServerInputs {
            ...
            configured_port: "49311".to_owned(),
            profile_root: "D:/P4475/Profile".to_owned(),
            client_path: "D:/Games/KartRider_P4475".to_owned(),
            client_data_dir: "D:/Games/KartRider_P4475/Data".to_owned(),
            allow_remote_profile_creation: true,
            ...
            item_probability_xml: "D:/P4475/item-probability.xml".to_owned(),
        ...
        let restored = P4475GuiApp::new(PathBuf::new(), Some(&storage));
    ...
    fn dropping_the_gui_cancels_its_active_worker_before_launch() {
        let cancellation = ConnectorCancellation::new();
        let mut app = P4475GuiApp::new(PathBuf::from("p4475-test.log"), None);
