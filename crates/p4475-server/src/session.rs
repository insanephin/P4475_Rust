...
use p4475_core::{
    ...
    floater_physics::{intrinsic_floater_codes, p4475_floater_spec},
    floater_protocol::{
    ...
    kart_level_physics::p4475_kart_level_spec,
    kart_level_protocol::{
    ...
    kart_physics::{
        KartPhysicsBuildError, P4475_MODERN_SPEED_TYPE_COUNT, P4475KartPhysicsSnapshot,
        build_p4475_kart_physics_block,
    },
    lobby_protocol::{
    ...
    login::{
        LegacyTime, LoginError, P4475_OBSERVER_MASTER_PMAP, P4475_OBSERVER_PMAP, PrLoginFields,
    ...
    packet::PacketError,
    plant_physics::{P4475PlantGameMode, apply_p4475_plant_part},
    race_protocol::{
...
pub enum LoginSessionError {
    ...
    #[error(
        "P4475 static channel catalog has no record for game type {game_type} and preferred channel {preferred_channel}"
    )]
    UnsupportedChannel {
        game_type: u8,
...
impl ProfileCoordinator {
    ...
    async fn load_myroom_wire_projection_for_operation(
        ...
        let mut players: [Option<MyRoomPlayerSlot>;
            p4475_core::myroom_protocol::MYROOM_SLOT_COUNT] = array::from_fn(|_| None);
    ...
    async fn complete_rider_school_step(
        ...
        tracing::debug!(
            ...
            "persisted P4475 rider-school test completion"
        );
        Ok((
            ProfileSnapshot {
                profile,
    ...
    async fn update_favorite_items(
        &self,
        nickname: String,
        changes: Vec<p4475_core::item_state_protocol::FavoriteItemChange>,
    ...
    fn validate_main_emblems(
        &self,
        request: p4475_core::myroom_protocol::UpdateMainEmblemRequest,
    ) -> Result<
    ...
    async fn equip_plant_part(
        ...
        admission: ProfileJobAdmission,
    ) -> Result<(p4475_core::inventory::PlantExcRecord, ProfileLanePermit), LoginSessionError> {
        #[cfg(test)]
        let blocking_update_hook = self.blocking_update_hook.clone();
        let completed = admission
    ...
    async fn equip_x_part(
        ...
        admission: ProfileJobAdmission,
    ) -> Result<(p4475_core::inventory::PartsExcRecord, ProfileLanePermit), LoginSessionError> {
        #[cfg(test)]
        let blocking_update_hook = self.blocking_update_hook.clone();
        let completed = admission
                ...
                let applied = match mutation {
                    ...
                    Err(EquipmentProfileError::State(error)) => {
                        ...
                        tracing::warn!(
                            ...
                            %error,
                            "rejected P4475 Floater mutation without closing the session"
                        );
                ...
                let state = match mutation {
                    ...
                    Some(Err(EquipmentProfileError::State(error))) => {
                        tracing::warn!(
                            nickname = subject.nickname(),
                            %error,
                            "rejected P4475 kart-level mutation without closing the session"
                        );
...
struct FloaterProfileResult {
    result_code: i32,
    response_state: p4475_core::inventory::TuneExcRecord,
    equipment: EquipmentExceptions,
}

struct AppliedFloaterMutation {
    response_state: p4475_core::inventory::TuneExcRecord,
    persisted_state: p4475_core::inventory::TuneExcRecord,
    durability_warning: Option<EquipmentStateError>,
}

fn direct_floater_mutation(
    outcome: EquipmentMutationOutcome<p4475_core::inventory::TuneExcRecord>,
...
fn upsert_plant_record(
    ...
    record: p4475_core::inventory::PlantExcRecord,
) {
...
fn upsert_parts_record(
    equipment: &mut EquipmentExceptions,
    record: p4475_core::inventory::PartsExcRecord,
) {
    if let Some(existing) = equipment
        .parts
        .iter_mut()
...
fn upsert_tune_record(
    equipment: &mut EquipmentExceptions,
    record: p4475_core::inventory::TuneExcRecord,
) {
    if let Some(existing) = equipment
        .tune
        .iter_mut()
...
fn floater_state_for(
    equipment: &EquipmentExceptions,
    target: KartInstance,
) -> p4475_core::inventory::TuneExcRecord {
    ...
    equipment
        ...
        .copied()
        .unwrap_or(p4475_core::inventory::TuneExcRecord {
            id: target.kart_id,
            serial,
            tune1: 0,
            tune2: 0,
...
fn upsert_kart_level_record(
    equipment: &mut EquipmentExceptions,
    record: p4475_core::inventory::KartLevelExcRecord,
) {
    if let Some(existing) = equipment
        .kart_level
        .iter_mut()
    ...
    match event {
        ClientEvent::NewCareerItemState(report) => {
            tracing::trace!(
                ...
                "accepted bounded P4475 career item-state telemetry"
            );
        }
        ClientEvent::ReportUdpReconnect => {
            ...
            tracing::debug!(
                ...
                "authorized same-generation P4475 UDP endpoint rebind"
            );
        }
    }
    Ok(Vec::new())
    ...
    match request {
        SinglePlayerRequest::StartSingle(request) => {
            ...
            tracing::trace!(
                ...
                "accepted complete P4475 single-player start proof"
            );
            Ok(Vec::new())
        }
        SinglePlayerRequest::UseItem(request) => {
            ...
            tracing::trace!(
                ...
                "accepted currently non-mutating P4475 single-player item event"
            );
            Ok(Vec::new())
        }
        SinglePlayerRequest::KartSpec(request) => {
            ...
            let physics = selected_physics_metadata(
                ...
                PhysicsSelection {
                    ...
                    plant_game_mode: P4475PlantGameMode::Unknown,
...
async fn handle_challenger_kart_spec(
    ...
    let physics = selected_physics_metadata(
        ...
        PhysicsSelection {
            ...
            flying_pet_id: request.flying_pet_id,
            requested_speed_type: request.speed_type,
            plant_game_mode: P4475PlantGameMode::Speed,
...
async fn handle_complete_challenger(
    ...
    tracing::trace!(
        ...
        "accepted complete P4475 challenger result proof"
    );
...
async fn handle_start_time_attack(
    ...
    let physics = selected_physics_metadata(
        ...
        PhysicsSelection {
            ...
            plant_game_mode: P4475PlantGameMode::TimeAttack,
...
async fn handle_matching_request(
    ...
    match request {
        MatchingRequest::Start(_) => {
            ...
            tracing::debug!(
                nickname = %identity.nickname,
                "accepted P4475 matching envelope; returning complete empty-match create state"
            );
        ...
        MatchingRequest::Cancel => {
            tracing::trace!(
                nickname = %identity.nickname,
                "accepted P4475 matching cancellation"
            );
            Ok(Vec::new())
        }
    }
...
async fn handle_get_rider_info(
    ...
    // P4475 Rust stores profile directories under a canonical key and does
...
async fn handle_shop_buy_failure(
        ...
        Err(error) => match &error {
            ShopProtocolError::Packet(_) | ShopProtocolError::TrailingBytes { .. } => {
                tracing::debug!(
                    packet_length = packet.len(),
                    %error,
                    "dropping malformed P4475 shop-buy request"
                );
                return Ok(Vec::new());
            }
    ...
    tracing::debug!(
        ...
        packet_length = packet.len(),
        "rejecting P4475 shop-buy request fail-closed"
    );
...
async fn handle_club_channel_switch(
    ...
    tracing::debug!(
        ...
        "accepted local P4475 Club channel transition"
    );
...
async fn handle_login(
    ...
    tracing::info!(
        ...
        "authenticated P4475 account role"
    );
    ...
    let payload = match request {
        ...
        LobbyRequest::StartRoom => {
            ...
            LobbyCommandPayload::StartRoom(
                StartRoomPlan::new(vec![P4475_FALLBACK_TRACK_ID], Vec::new())
        ...
        RaceRequest::GameControl => {
            ...
            if let Some(snapshot) = parse_game_control_finish_snapshot(&control)? {
                tracing::trace!(
                    ...
                    "decoded non-authoritative P4475 finish snapshot"
                );
            }
    ...
    match request {
        EquipmentRequest::SetRiderItems => {
            if packet.len() < size_of::<u32>() + RIDER_ITEM_SNAPSHOT_WIRE_LENGTH {
                ...
                tracing::debug!(
                    ...
                    "ignored truncated P4475 rider-equipment request"
                );
                return Ok(Vec::new());
            }
        ...
        EquipmentRequest::EquipPlantPart => {
            let request = match parse_equip_plant_part(packet) {
                Ok(request) => request,
                Err(error) => {
                    tracing::debug!(%error, "rejected malformed P4475 plant-part request");
                    let identity = world.authorize_identity().await?;
                    let _ = context.profile_for(&identity)?;
...
async fn update_rider_equipment(
    ...
    tracing::info!(
        ...
        flying_pet_spec_available = selection.flying_pet == 0
            ...
            .is_some(),
        "accepted P4475 rider equipment"
    );
    let admission = profiles
        .admit_for_operation(
            world.operation(),
    ...
    tracing::info!(
        ...
        "resolved P4475 rider equipment physics after durable selection"
    );
    context.bind_profile(after, profile);
    drop(lane);
    Ok(())
...
async fn equip_plant_part(
    ...
    let (record, lane) = match profiles.equip_plant_part(request, admission).await {
        Ok(result) => result,
        Err(error) => {
            tracing::warn!(%error, "failed to persist P4475 plant-part selection");
            return Ok(vec![serialize_equip_tuning_failure()]);
        }
    };
    ...
    tracing::info!(
        ...
        refreshed_room_physics,
        "installed durable P4475 plant part in the bound physics cache"
    );
    drop(lane);
...
async fn equip_x_part(
    ...
    let (record, lane) = match profiles.equip_x_part(request, admission).await {
        Ok(result) => result,
        Err(error) => {
            tracing::warn!(%error, "failed to persist P4475 X-part selection");
    ...
    tracing::info!(
        ...
        item_id = request.item_id,
        refreshed_room_physics,
        "installed durable P4475 X-part in the bound physics cache"
    );
    drop(lane);
...
async fn handle_floater_request(
    ...
    tracing::info!(
        ...
        refreshed_room_physics,
        "processed durable P4475 Floater request"
    );
    drop(lane);
...
async fn handle_kart_level_request(
    ...
    tracing::info!(
        ...
        refreshed_room_physics,
        "processed durable P4475 legacy kart-level request"
    );
    drop(lane);
...
struct PhysicsSelection {
    ...
    plant_game_mode: P4475PlantGameMode,
    apply_profile_equipment: bool,
}

impl PhysicsSelection {
    fn for_room(profile: &Profile, plant_game_mode: P4475PlantGameMode) -> Self {
        Self {
...
fn room_physics_metadata(
    ...
    let mut speed = selected_physics_metadata(
        ...
        Some(equipment),
        catalog,
        PhysicsSelection::for_room(profile, P4475PlantGameMode::Speed),
    )?;
    let item = selected_physics_metadata(
        profile,
        Some(equipment),
        catalog,
        PhysicsSelection::for_room(profile, P4475PlantGameMode::Item),
    )?;
...
fn selected_physics_metadata(
    ...
    let mut snapshot = P4475KartPhysicsSnapshot::csharp_s7_baseline();
    if let Some(speed) =
        p4475_core::kart_physics::P4475SpeedSpecSnapshot::csharp_modern(requested_speed_type)
    {
    ...
    let base_resolution = if kart_id == 0 {
        ...
        if let Some(spec) = catalog.kart_spec(kart_id) {
            ...
            if let Some(xun) = catalog.kart_xun_profile(kart_id) {
                snapshot.v2 = xun.apply_p4475_compatibility(&mut snapshot.kart);
            }
    ...
    if floater_codes != [0; 3] {
        ...
        snapshot.exc.tune = p4475_floater_spec(codes)
    ...
    let block = build_p4475_kart_physics_block(&snapshot)?;
    let variants = (0_u8..u8::try_from(P4475_MODERN_SPEED_TYPE_COUNT)
        .expect("modern speed-type count fits in a protocol byte"))
        .map(|speed_type| {
            let mut variant = snapshot;
            variant.speed_type = speed_type;
            variant.speed =
                p4475_core::kart_physics::P4475SpeedSpecSnapshot::csharp_modern(speed_type)
                    .expect("every protocol speed byte from 0 through 8 has a modern preset");
            build_p4475_kart_physics_block(&variant)
        })
...
fn selected_profile_floater_codes(
    ...
    p4475_floater_spec(codes).map(|_| codes)
}

fn apply_profile_exception_physics(
    snapshot: &mut P4475KartPhysicsSnapshot,
    equipment: Option<&EquipmentExceptions>,
    kart_id: u16,
    kart_serial: u16,
    plant_game_mode: P4475PlantGameMode,
    ...
    {
        let codes = [tune.tune1, tune.tune2, tune.tune3];
        if let Some(spec) = p4475_floater_spec(codes) {
        ...
        .find(|record| record.id == kart_id && record.serial == kart_serial)
    {
        ...
        {
            if !apply_p4475_plant_part(&mut snapshot.exc, category, item_id, plant_game_mode) {
        ...
        .find(|record| record.id == kart_id && record.serial == kart_serial)
    {
        ...
        if let Some(spec) = p4475_kart_level_spec(levels) {
...
fn room_participant_from_profile_with_p2p_port(
    ...
    let observer = matches!(
        profile.rider.pmap,
        P4475_OBSERVER_PMAP | P4475_OBSERVER_MASTER_PMAP
    );
    let observer_master = profile.rider.pmap == P4475_OBSERVER_MASTER_PMAP;
...
async fn update_rider_school_data_admitted(
    ...
    if !(1..=startup::P4475_RIDER_SCHOOL_MAX_STEP).contains(&request.completed_step) {
...
async fn update_rider_school_level_admitted(
    ...
    let expected = current
        .saturating_add(1)
        .min(startup::P4475_RIDER_SCHOOL_LEVEL);
...
fn startup_response(
    ...
    // P4475 does not gate the active PRO pair from the date embedded in
    ...
    let response = match request {
        ...
        StartupRequest::RiderSchoolProgress => {
            let records = startup::P4475_RACING_MASTER_TRACK_IDS.map(|track| {
                profile
                    .time_attack_records
                    .get(&track)
                    .copied()
...
fn profile_game_options(options: &p4475_profile::GameOptions) -> startup::GameOptions {
    startup::GameOptions {
...
fn apply_game_options(destination: &mut p4475_profile::GameOptions, request: &PqUpdateGameOption) {
...
fn rider_school_pro_pair_index_for_year_month(year: i32, month: u32) -> usize {
    ...
    usize::try_from(
        (odd_month_count - 1).rem_euclid(
            i32::try_from(startup::P4475_RIDER_SCHOOL_PRO_PAIR_COUNT)
...
mod tests {
    ...
    use p4475_core::{
        ...
        floater_physics::p4475_floater_spec,
        floater_protocol::{
        ...
        kart_physics::{P4475KartPhysicsSnapshot, build_p4475_kart_physics_block},
    ...
    use super::{
        ...
        myroom_player_slot_from_profile, myroom_profile_presentation, p4475_kart_level_spec,
    ...
    fn test_catalog() -> Arc<CatalogInventory> {
        const KNOWN_KART_ID: u16 = 981;
        const MISSING_SPEC_KART_ID: u16 = 1_008;
        const XUN_KART_ID: u16 = 1_454;
        ...
        let xml = format!(
            r#"<KartCatalog formatVersion="3" protocolVersion="5136" region="kr">
                <Names>
                    ...
                    <Kart id="{XUN_KART_ID}" name="sessionXunKart" />
                </Names>
                <Specs>
    ...
    fn exact_get_rider_info_request(target_nickname: &str, mode: u8) -> Vec<u8> {
        ...
        packet
            ...
            .expect("test nickname fits the P4475 length prefix");
    ...
    fn validate_retained_gap_packet(hash: u32, packet: &[u8]) -> bool {
        ...
        if super::classify_item_state_request(hash)
            == Some(p4475_core::item_state_protocol::ItemStateRequest::LockedItemUpdate)
        {
    ...
    async fn none_cleared_profile_advances_and_survives_reload() {
        ...
        store
            .update("LicenseStarter", |profile| {
                profile.rider_school = p4475_profile::RiderSchoolProgress::NONE_CLEARED;
            })
            .unwrap();
        ...
        assert_eq!(
            reply,
            vec![p4475_core::startup::serialize_pr_update_rider_school_level(
                1
            )]
        );
        ...
        assert_eq!(
            ...
            p4475_profile::RiderSchoolProgress {
                level: 1,
    ...
    async fn l1_profile_exposes_test_records_and_unlocks_pro_missions_without_emblem_state() {
        ...
        store
            .update("L1NotPro", |profile| {
                profile.rider_school = p4475_profile::RiderSchoolProgress::L1;
                for (track, record) in p4475_core::startup::P4475_RACING_MASTER_TRACK_IDS
                    .into_iter()
        ...
        let mut reward = PacketWriter::named(p4475_core::startup::REWARD_PRO_EMBLEM_REQUEST_NAME);
        reward.write_u16(p4475_core::startup::P4475_RACING_MASTER_EMBLEM_ID);
        ...
        assert_eq!(
            &reward_reply[..4],
            &p4475_core::startup::REWARD_PRO_EMBLEM_REPLY_HASH.to_le_bytes()
        );
        ...
        assert_eq!(
            persisted.profile.rider_school,
            p4475_profile::RiderSchoolProgress::L1,
    ...
    async fn connector_role_override_is_durable_across_channel_profile_reload() {
        ...
        let (observer, _equipment, observer_lane) = profiles
            .load_with_equipment_and_pmap(
                "Caster".to_owned(),
                true,
                Some(p4475_core::login::P4475_OBSERVER_MASTER_PMAP),
                observer_admission,
            )
            .await
            .unwrap();
        assert_eq!(
            ...
            p4475_core::login::P4475_OBSERVER_MASTER_PMAP
        );
        drop(observer_lane);
        ...
        let (regular, _equipment, regular_lane) = profiles
            .load_with_equipment_and_pmap(
                "Caster".to_owned(),
                true,
                Some(p4475_core::login::P4475_REGULAR_PMAP),
                regular_admission,
            )
            .await
            .unwrap();
        ...
        let (league, _equipment, league_lane) = profiles
            .load_with_equipment_and_pmap(
                "Caster".to_owned(),
                true,
                Some(p4475_core::login::P4475_ANONYMOUS_LEAGUE_PMAP),
                league_admission,
            )
            .await
            .unwrap();
        assert_eq!(
            ...
            p4475_core::login::P4475_ANONYMOUS_LEAGUE_PMAP
        );
        drop(league_lane);
    ...
    #[test]
    #[ignore = "requires external packet traces via P4475_PACKET_TRACE_DIR"]
    fn external_retained_packet_corpus_matches_the_dispatch_domains() {
        let Some(directory) = std::env::var_os("P4475_PACKET_TRACE_DIR") else {
            eprintln!("P4475_PACKET_TRACE_DIR is not set; skipping local corpus audit");
            return;
        };
    ...
    async fn every_remaining_captured_telemetry_family_is_a_bounded_no_reply_operation() {
        ...
        let mut ai = vec![0; p4475_core::telemetry_protocol::GAME_AI_REPORT_LENGTH];
        ai[..4].copy_from_slice(&adler32::packet_hash("GameAiReportPacket").to_le_bytes());
        reports.push(ai);

        let mut game = vec![0; p4475_core::telemetry_protocol::GAME_REPORT_LENGTH];
    ...
    async fn captured_single_player_time_attack_flow_replaces_an_unfinished_attempt() {
        ...
        assert_eq!(
            kart_reply[0].len(),
            p4475_core::single_player_protocol::KART_SPEC_REPLY_LENGTH
        );
        ...
        let expected_physics = selected_physics_metadata(
            ...
            PhysicsSelection {
                ...
                plant_game_mode: p4475_core::plant_physics::P4475PlantGameMode::TimeAttack,
                apply_profile_equipment: true,
            },
        )
        .unwrap()
        ...
        assert_eq!(
            start_reply[0].len(),
            p4475_core::single_player_protocol::START_TIME_ATTACK_REPLY_LENGTH
        );
        ...
        // P4475 can return to the time-attack menu without sending
        ...
        assert_eq!(
            finish_reply,
            vec![
                p4475_core::single_player_protocol::serialize_finish_time_attack_reply(2, 0, 1, 1)
            ]
        );
        ...
        assert_eq!(loaded.profile.rider.rp, p4475_profile::DEFAULT_RP);
        assert_eq!(
    ...
    async fn pro_license_projection_unlocks_the_captured_challenge_time_attack_sequence() {
        ...
        assert_eq!(
            school_data[4],
            p4475_core::startup::P4475_RIDER_SCHOOL_LEVEL
        );
        assert_eq!(
            school_data[5],
            p4475_core::startup::P4475_RIDER_SCHOOL_MAX_STEP
        );
        assert_eq!(
        ...
        assert_eq!(pro[6], p4475_core::startup::P4475_RIDER_SCHOOL_LEVEL);
    ...
    async fn challenger_battle_start_spec_and_completion_are_one_live_session_flow() {
        ...
        assert_eq!(
            start_reply,
            vec![
                p4475_core::single_player_protocol::serialize_start_challenger_reply(
                    p4475_core::single_player_protocol::StartChallengerRequest {
                        stage_id: 40,
                        stage_context: 0,
                        game_type: 0,
                        kart_id: 787,
        ...
        assert_eq!(
            spec_reply[0].len(),
            p4475_core::single_player_protocol::CHALLENGER_KART_SPEC_REPLY_LENGTH
        );
        assert_eq!(
            &spec_reply[0][..4],
        ...
        assert_eq!(
            complete_reply,
            vec![p4475_core::single_player_protocol::serialize_complete_challenger_reply(2, 7)]
        );
        ...
        assert!(matches!(
            ...
            Err(LoginSessionError::ItemStateProtocol(
                ItemStateProtocolError::TrailingBytes {
                    name: p4475_core::item_state_protocol::LOCKED_ITEM_GET_REQUEST_NAME,
                    count: 1,
                }
            ))
        ));
    ...
    async fn rider_info_lookup_returns_existing_profile_without_creating_missing_riders() {
        ...
        store
            .update("ExistingTarget", |profile| {
                ...
                profile.rider_school = p4475_profile::RiderSchoolProgress::L2;
            })
            .unwrap();
    ...
    async fn endpoint_reports_persist_only_p2p_port_and_keep_game_udp_isolated() {
        ...
        assert!(matches!(
            dispatch_packet(&services, &malformed, &mut context).await,
            Err(LoginSessionError::ChannelProtocol(
                p4475_core::channel::ChannelError::TrailingBytes { .. }
            ))
        ));
        assert_eq!(
            ProfileStore::new(profile_root.path())
    ...
    async fn rejected_start_keeps_session_alive_and_uses_actor_reply() {
        ...
        assert_eq!(
            ...
            p4475_core::adler32::packet_hash("GrReplyStartPacket")
        );
    ...
    fn xun_catalog_metadata_is_projected_into_the_p4475_physics_block() {
        let catalog = test_catalog();
        let mut profile = Profile::default();
        ...
        expected_snapshot.v2 = xun.apply_p4475_compatibility(&mut expected_snapshot.kart);
    ...
    fn mixed_grade_floater_codes_are_converted_into_selected_kart_physics() {
        ...
        let baseline = {
            ...
            snapshot.exc.tune = p4475_core::kart_physics::P4475TuneSpecSnapshot::default();
            build_p4475_kart_physics_block(&snapshot).unwrap()
        };
    ...
    fn intrinsic_floater_codes_apply_to_time_attack_without_a_profile_sidecar() {
        ...
        .unwrap();
        let mut expected = P4475KartPhysicsSnapshot::csharp_s7_baseline();
        expected.exc.tune = p4475_floater_spec([603, 903, 702]).unwrap();

        assert_eq!(
            resolved.block,
            build_p4475_kart_physics_block(&expected).unwrap()
        );
    }
    ...
    async fn myroom_owner_item_read_keeps_parts_without_karts_and_types_sidecar_errors() {
        ...
        assert_eq!(
            u32::from_le_bytes(packets[0][..4].try_into().unwrap()),
            p4475_core::adler32::packet_hash(OWNER_ITEM_NAME)
        );
        assert_eq!(
    ...
    async fn legacy_kart_upgrade_is_certain_non_consuming_durable_and_preloaded() {
        ...
        assert_eq!(
            handle_kart_level(&world, &profiles, session, &probability, &mut context,)
                .await
                .unwrap(),
            vec![p4475_core::kart_level_protocol::serialize_kart_level_up_probability_success()]
        );
        ...
        let physics =
            room_physics_metadata(&bound_profile, &bound_equipment, Some(catalog.as_ref()))
                .unwrap();
        let mut expected = P4475KartPhysicsSnapshot::csharp_s7_baseline();
        expected.exc.kart_level = p4475_kart_level_spec([10, 10, 10, 5]).unwrap();
        assert_eq!(
            physics.block,
            build_p4475_kart_physics_block(&expected).unwrap()
        );
    ...
    async fn unlimited_floater_policy_does_not_require_catalog_or_kart_grants() {
        ...
        let physics =
            room_physics_metadata(context.profile_for(&identity).unwrap(), equipment, None)
                .unwrap();
        let mut expected = P4475KartPhysicsSnapshot::csharp_s7_baseline();
        expected.exc.tune = p4475_floater_spec([603, 703, 903]).unwrap();
        assert_eq!(
            physics.block,
            build_p4475_kart_physics_block(&expected).unwrap()
        );
    ...
    async fn black_floater_is_non_consuming_durable_preloaded_and_applied_to_room_physics() {
        ...
        let physics =
            room_physics_metadata(&bound_profile, &bound_equipment, Some(catalog.as_ref()))
                .unwrap();
        let mut expected = P4475KartPhysicsSnapshot::csharp_s7_baseline();
        expected.exc.tune = p4475_floater_spec([603, 0, 903]).unwrap();
        assert_eq!(
            physics.block,
            build_p4475_kart_physics_block(&expected).unwrap()
        );
