...
use p4475_profile::CatalogInventory;
...
///
/// P4475 consumes the first four values as target-speed factor, base-speed
...
impl Default for BasicAiParameters {
    fn default() -> Self {
        Self::try_from(DEFAULT_SPEED_BASIC_AI_PARAMETER_VALUES)
            .expect("the stock P4475 basic-AI parameter vector is valid")
    }
}
    ...
    fn default() -> Self {
        Self {
            speed: BasicAiParameters::try_from(DEFAULT_SPEED_BASIC_AI_PARAMETER_VALUES)
                .expect("the stock P4475 speed-AI vector is valid"),
            item: BasicAiParameters::try_from(DEFAULT_ITEM_BASIC_AI_PARAMETER_VALUES)
                .expect("the stock P4475 item-AI vector is valid"),
        }
    }
}
...
///
/// The visible S grade is not identical to the P4475 protocol speed byte:
...
/// Selects the stock P4475 PRO license mission pair.
///
