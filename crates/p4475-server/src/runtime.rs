...
use p4475_core::dataraw_manifest::{DataRawManifest, DataRawManifestError};
use p4475_profile::{
...
use p4475_rho5::{Rho5Directory, Rho5Error, Rho5Limits};
use thiserror::Error;
use tokio::{
...
pub enum ServerError {
    ...
    #[error("advertised IPv4 address {0} cannot be reached by a P4475 client")]
...
impl BoundServer {
    /// Transactionally binds the stock P4475 transports and the private XUN
    ...
    pub fn start(self) -> Result<ServerHandle, ServerError> {
        ...
        tracing::info!(
            ...
            "P4475 server configuration validated and transports started"
        );
...
mod tests {
    ...
    use p4475_profile::{EquipmentExceptions, Profile, ProfileStore, rider_item_snapshot};
    use tokio::{
            ...
            .room_protocol(
                ...
                RoomCommandPayload::Create {
                    ...
                    participant: RoomParticipant {
                        ...
                        kart_physics: P4475KartPhysicsBlock::from([0; 235]),
    ...
    #[tokio::test]
    #[ignore = "requires a local proprietary P4475 installation via P4475_DATA_DIR"]
    async fn local_client_data_extracts_and_parses_all_emblem_ids() {
        let Some(directory) = std::env::var_os("P4475_DATA_DIR") else {
            return;
        };
