...
/// P4475 client processing flags for a zlib-compressed payload encrypted with
/// the normal RHO5 stream plus the additional first-0x400-byte stream.
///
/// Stock Korean P4475 archives use this value for every populated entry.  A
/// zero value makes the native client expose the stored ciphertext directly
/// to resource consumers instead of decoding it.
pub const P4475_PACKED_ENTRY_FLAGS: i32 = 0x0000_0007;
...
impl Rho5Directory {
    /// Scans all regular `*.rho5` files in `directory` using the KR P4475 keys.
    ...
    /// Extracts an authenticated P4475 entry while accepting the stock
...
/// Computes the encrypted header and file-table positions for a P4475 RHO5 name.
