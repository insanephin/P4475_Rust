...
use crate::{
    DATA_ALIGNMENT, DOUBLE_ENCRYPTED_PREFIX, P4475_PACKED_ENTRY_FLAGS, RHO5_VERSION, Rho5Error,
...
impl Rho5Writer {
    ...
    pub fn encode(
        ...
        for entry in &self.entries {
            ...
            if entry.flags != P4475_PACKED_ENTRY_FLAGS {
                return Err(Rho5WriteError::UnsupportedFlags {
                    path,
                    actual: entry.flags,
                    expected: P4475_PACKED_ENTRY_FLAGS,
                });
            }
...
pub enum Rho5WriteError {
    ...
    ArithmeticOverflow,
    #[error(
        "RHO5 output entry {path:?} has flags {actual:#010x}; the P4475 packed codec requires {expected:#010x}"
    )]
    UnsupportedFlags {
        path: String,
        actual: i32,
...
mod tests {
    ...
    use crate::{P4475_PACKED_ENTRY_FLAGS, Rho5Directory, Rho5Limits, Rho5Region};
    ...
    fn deterministic_archive_round_trips_through_stock_reader() {
        ...
        assert_eq!(decoded.entries()[0].flags(), P4475_PACKED_ENTRY_FLAGS);
        assert_eq!(decoded.entries()[1].flags(), P4475_PACKED_ENTRY_FLAGS);
        assert_eq!(
